...
class PlanWorkspace:
    def __init__(
        ...
        if "plan_num_kept_patches" in self.cfg_dict and self.cfg_dict["plan_num_kept_patches"] is not None:
            self.plan_num_kept_patches = self.cfg_dict["plan_num_kept_patches"]
        else:
            self.plan_num_kept_patches = None
        
        self.saved_folder = self.cfg_dict.get('saved_folder', '.')
        ...
        self.planner = hydra.utils.instantiate(
            ...
            drop=drop,
            plan_num_kept_patches=self.plan_num_kept_patches,
        )
    ...
    def sample_traj_segment_from_dset(self, traj_len):
        ...
        valid_traj = [
            self.dset[i][0]["proprio"].shape[0]
            for i in range(len(self.dset))
            if self.dset[i][0]["proprio"].shape[0] >= traj_len
        ]
        if len(valid_traj) == 0:
        ...
        for i in range(self.n_evals):
            ...
            while max_offset < 0:  # filter out traj that are not long enough
                ...
                max_offset = obs["proprio"].shape[0] - traj_len
            state = state.numpy()
...
def load_model(
    ...
    if "proprio_encoder" not in result or "action_encoder" not in result:
        raise ValueError("Action/proprio encoder not found in model checkpoint")

    if train_cfg.has_decoder and "decoder" not in result:
        base_path = os.path.dirname(os.path.abspath(__file__))
        if train_cfg.env.decoder_path is None:
            raise ValueError("Decoder missing from checkpoint and no decoder_path is set")
    ...
    extra_kwargs = {}
    if drop:
        if plan_num_kept_patches is None:
            raise ValueError("drop=True requires plan_num_kept_patches")
        train_cfg_cpy.model._target_ = train_cfg_cpy.model._target_.replace(
            "VWorldModel", "VWorldModelDrop"
        )
        train_cfg_cpy.has_decoder = False
        decoder = None
        extra_kwargs["plan_num_kept_patches"] = plan_num_kept_patches
    elif plan_num_kept_patches is not None:
        raise ValueError("plan_num_kept_patches is only supported with drop=True.")
...
def planning_main(cfg_dict):
    ...
    if cfg_dict.get("wandb_logging", False):
        project_name = f"plan_{cfg_dict['planner']['name']}"
        wandb_run = wandb.init(project=project_name, config=cfg_dict)
        wandb.run.name = "{}".format(output_dir.split("plan_outputs/")[-1])
    else:
        wandb_run = None

    ckpt_root = cfg_dict.get("ckpt_root")
    if ckpt_root is None:
        ckpt_base_path = cfg_dict["ckpt_base_path"]
        if os.path.basename(os.path.normpath(ckpt_base_path)) == "outputs":
            ckpt_root = ckpt_base_path
        else:
            ckpt_root = os.path.join(ckpt_base_path, "outputs")
    model_path = os.path.join(ckpt_root, cfg_dict["model_name"])
    ...
    model = load_model(
        ...
        num_action_repeat,
        device=device,
        plan_num_kept_patches=cfg_dict.get("plan_num_kept_patches"),
        drop=drop,
    )
    model_cfg_env_name = model_cfg.env.name

    # use dummy vector env for wall and deformable envs
    if model_cfg_env_name == "wall" or model_cfg_env_name == "deformable_env" or model_cfg_env_name == "point_maze":
