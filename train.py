...
class Trainer:
    ...
    def save_ckpt(self, step=None):
        ...
        if self.accelerator.is_main_process:
            ...
            # Save an additional checkpoint keyed by step or epoch.
            if step is not None:
    ...
    def init_models(self):
        ...
        self.model = hydra.utils.instantiate(
            ...
            drop_rate_ub=self.cfg.drop_rate_ub
        )
    ...
    def train(self):
        ...
        for i, data in enumerate(tqdm(dataloader, desc=f"Epoch {self.epoch} Train")):
            obs, act, _ = data
    ...
    def openloop_rollout(
        ...
        for idx in range(num_rollout):
            ...
            while not valid_traj:
                ...
                if rand_start_end:
                    if obs["proprio"].shape[0] > min_horizon * self.cfg.frameskip + 1:
                        start = np.random.randint(
                            0,
                            obs["proprio"].shape[0] - min_horizon * self.cfg.frameskip - 1,
                        )
                    else:
                        start = 0
                    max_horizon = (obs["proprio"].shape[0] - start - 1) // self.cfg.frameskip
                    ...
                    valid_traj = True
                    start = 0
                    horizon = (obs["proprio"].shape[0] - 1) // self.cfg.frameskip
