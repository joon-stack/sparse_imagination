...
class GDPlanner(BasePlanner):
    ...
    def plan(self, obs_0, obs_g, actions=None):
        ...
        if self.drop and self.plan_num_kept_patches is not None:
            # When used as MPC's sub-planner, the world model is already configured.
            # Only perform token selection when GD is used standalone.
            if not hasattr(self.wm, 'keep_patches_idx') or self.wm.keep_patches_idx is None:
                patch_num = 196
                # Random Drop
                print("INFO: GDPlanner Random Drop")
                keep_patches_idx = torch.randperm(patch_num)[: self.plan_num_kept_patches]
                drop_patches_idx = torch.tensor([i for i in range(patch_num) if i not in keep_patches_idx])
                
                self.wm.keep_patches_idx = keep_patches_idx
                self.wm.drop_patches_idx = drop_patches_idx
                    
                print(f"INFO: GDPlanner token selection - keep_patches: {keep_patches_idx.shape}")
